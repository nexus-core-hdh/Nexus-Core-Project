...
interface Props {
  accountId: number;
  tab: string; // matches backend SATELLITES key, e.g. "contacts"
  fields: SatelliteField[];
  addLabel?: string;
  readOnly?: boolean;
}

export function SatelliteGridTab({ accountId, tab, fields, addLabel = "Add Row", readOnly = false }: Props) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const r: any = await legacyErpApi.accounts.listTab(accountId, tab);
      setRows(Array.isArray(r) ? r : []);
    } finally {
      setLoading(false);
    }
  };
  useEffect(() => { load(); }, [accountId, tab]);
  ...
  const save = async () => {
    setSaving(true);
    try {
      await legacyErpApi.accounts.createTabRow(accountId, tab, form);
      toast.success("Added");
      setOpen(false);
      setForm({});
      load();
  ...
  const remove = async (id: number) => {
    try {
      await legacyErpApi.accounts.removeTabRow(accountId, tab, id);
      load();
    } catch (e: any) {
      toast.error(e.message);
    }
  ...
  // `tab` on top of the base key so different tabs never collide in tablePreferences. The first
  // field is treated as the row's identifying column and stays pinned first/unhideable.
  ...
  const gridColumns = useGridColumns<string>({
    storageKey: `currentAccountsSatelliteGrid_${tab}`,
