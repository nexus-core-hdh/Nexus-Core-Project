...
export default function TrimCardListPage() {
  ...
  const wrapTrimCardRow = (row: any, el: React.ReactNode) => {
    ...
    const withNav = cloneElement(el as React.ReactElement<any>, {
      ...
      className: cn((el as React.ReactElement<any>).props.className, "outline-none", selectedId === row.id && "bg-selected hover:bg-selected-hover", mode === "lookup" && "cursor-pointer"),
    });
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-5 p-6 lg:p-8">
      ...
      <ModuleHeader
        icon={Ribbon}
        title="Trim Card"
        subtitle={mode === "lookup" ? "Double-click or press Enter to select a Trim Card" : "Trim item master"}
        badges={
          !loading && (
