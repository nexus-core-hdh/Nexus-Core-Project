...
// table (see fi-receipt.service.ts's own header comment for why this is not the same entity as
// IM_Receipt). Reuses that screen's shared worklist infrastructure (lib/legacy-erp/worklist-types.ts,
// hooks/legacy-erp/use-worklist.ts, components/legacy-erp/worklist-design-modal.tsx) rather than
// duplicating it — only this screen's own table-config.ts and namespaced worklist storage key are
// new. This screen never writes on its own; every action reuses the existing
// /legacy-erp/financial-receipts route.
...
export default function FinancialReceiptMasterDataPage() {
  const router = useRouter();
  const [selectedTable, setSelectedTable] = useState<TableKey>("financial-receipt");
  ...
  // Customize Worklist — same generic per-user UserSettings.tablePreferences JSON blob
  ...
  // Unified column model for WorklistTable — this screen has no Standard/custom-worklist split
  // (unlike purchase-orders-list etc.): `columns` above is always just the resolved key list
  // (worklist fields, or raw row keys when no worklist is active), so every column maps the
  // same way. No sortable columns existed here before, so none are added now.
  ...
  return (
    <div className="mx-auto max-w-[1700px] space-y-4 p-4 lg:p-6">
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <span className="font-medium text-foreground">Financial Receipt & Master Data</span>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border shadow-sm">
        <WorklistTable
          ...
          rows={rows}
          storageKey="financialReceiptMasterData"
          ...
          emptyState={
            <Empty>
              <EmptyHeader>
                <EmptyMedia variant="icon">{searched ? <SearchX /> : <Landmark />}</EmptyMedia>
