...
interface Props {
   ...
   *  Parameters" form both read/write via tab="warehouse-parameters", but have different field
   *  sets) — without this, both instances would silently share one saved column layout despite
   *  being different grids. Defaults to `tab` (existing behavior) when omitted. */
  columnsKey?: string;
}
...
export function SatelliteGridTab({ itemId, tab, fields, addLabel = "Add Row", readOnly = false, columnsKey }: Props) {
  ...
  const gridColumns = useGridColumns<string>({
    storageKey: `yarnSatelliteGrid_${columnsKey ?? tab}`,
