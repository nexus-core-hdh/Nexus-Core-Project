...
// Documents half of the shared IM_ItemAttachment table (Type=1) — the Yarn Card screen has
// a dedicated "Picture Gallery" tab for Type=2, so unlike Current Account's combined
// AttachmentsTab, this one is documents-only. Same staging-before-save pattern.
export function AttachmentsTab({ itemId, readOnly = false }: Props) {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const docs: any = await legacyErpApi.yarnCards.listAttachments(itemId, "document");
    ...
    (async () => {
      ...
      for (const s of staged) {
        try {
          ...
          await legacyErpApi.yarnCards.uploadAttachment(itemId, { kind: "document", fileName: s.file.name, dataUrl });
  ...
  const uploadFiles = async (fileList: FileList | null) => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        await legacyErpApi.yarnCards.uploadAttachment(itemId, { kind: "document", fileName: file.name, dataUrl });
        successCount++;
      } catch (e: any) {
    ...
    <div className="space-y-4">
      {!itemId && (
        <div className="flex items-center gap-2 rounded-lg border border-dashed bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
          Files picked here are staged locally and uploaded automatically once you Save the yarn card.
        </div>
      )}
