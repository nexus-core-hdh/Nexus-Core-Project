...
async function fetchAttachmentBlob(itemId: number, attId: number) {
  ...
  const res = await fetch(legacyErpApi.yarnCards.attachmentContentUrl(itemId, attId), {
...
// Picture Gallery — the Type=2 half of the same IM_ItemAttachment table the Attachments
// tab uses (Type=1), rendered as a thumbnail grid instead of a document list, matching the
// legacy screen's separate "Picture Gallery" tab. No dedicated gallery infrastructure
// exists elsewhere in this codebase to plug into (verified beforehand) — this reuses the
// only real precedent, the account-attachments upload/thumbnail pattern, retargeted here.
export function PictureGalleryTab({ itemId, readOnly = false }: Props) {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const pics: any = await legacyErpApi.yarnCards.listAttachments(itemId, "picture");
    ...
    (async () => {
      ...
      for (const s of staged) {
        try {
          ...
          await legacyErpApi.yarnCards.uploadAttachment(itemId, { kind: "picture", fileName: s.file.name, dataUrl });
  ...
  const uploadFiles = async (fileList: FileList | null) => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        await legacyErpApi.yarnCards.uploadAttachment(itemId, { kind: "picture", fileName: file.name, dataUrl });
        successCount++;
      } catch (e: any) {
  ...
  const remove = async (attId: number) => {
    if (!itemId) return;
    try {
      await legacyErpApi.yarnCards.removeAttachment(itemId, attId);
      load();
    } catch (e: any) {
    ...
    <div className="space-y-4">
      ...
      {!itemId && (
        <div className="flex items-center gap-2 rounded-lg border border-dashed bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
          Pictures picked here are staged locally and uploaded automatically once you Save the yarn card.
        </div>
      )}
