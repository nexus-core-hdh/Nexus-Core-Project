...
// F2 lookup target for the General tab's Season field, plus a plain CRUD screen otherwise —
// see brand-cards/page.tsx for the identical pattern this mirrors.
export default function SeasonCardsPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const r = await plmApi.seasons.list({ branchId: user?.branchId });
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing) { await plmApi.seasons.update(editing, payload); toast.success("Updated"); }
      else { await plmApi.seasons.create(payload); toast.success("Created"); }
      setOpen(false); load();
    ...
    <div className="p-6">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle>{editing ? 'Edit' : 'Add'} Season</DialogTitle></DialogHeader>
