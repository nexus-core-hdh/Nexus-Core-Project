...
// F2 lookup target for the General tab's Brand field (see MasterAutocompleteField's
// `lookupPath` override) as well as a plain CRUD screen when opened normally — same page,
// same PlmCrudTable, `lookupMode`/`onSelectRow` only kick in when opened with ?mode=lookup.
export default function BrandCardsPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const r = await plmApi.brands.list({ branchId: user?.branchId });
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing) { await plmApi.brands.update(editing, payload); toast.success("Updated"); }
      else { await plmApi.brands.create(payload); toast.success("Created"); }
      setOpen(false); load();
