...
// Mirrors yarn-card-attachments.service.ts exactly, against IM_OrderReceiptAttachment
// instead of IM_ItemAttachment — a purpose-built attachments table that already exists in
// the migrated schema with its own OrderReceiptId FK (confirmed 0 rows, first writer). Not
// a reuse of IM_ItemAttachment/YarnCardAttachmentsService: that table's InventoryId column
// is semantically an IM_Item.RecId FK, and a Purchase Order is not an IM_Item row — reusing
// it would risk RecId collisions between two unrelated entities. Same Type=1(document)/
// Type=2(picture) convention as every other attachments table in this module.
...
@Injectable()
export class PurchaseOrderAttachmentsService {
  constructor(private readonly prisma: PrismaService) {}

  async list(orderReceiptId: number, kind: 'document' | 'picture') {
    ...
    const rows = kind === 'picture'
      ? await this.prisma.$queryRaw<any[]>(Prisma.sql`
          ...
          FROM "IM_OrderReceiptAttachment"
          WHERE "OrderReceiptId" = ${orderReceiptId} AND "Type" = ${type} AND "IsDeleted" = 0
  ...
  async upload(orderReceiptId: number, dto: UploadDto, userId: number) {
    ...
    const rows = await this.prisma.$queryRaw<any[]>(Prisma.sql`
      INSERT INTO "IM_OrderReceiptAttachment" ("OrderReceiptId", "Type", "FileName", "Attachment", "Thumbnail", "InUse", "InsertedAt", "InsertedBy", "IsDeleted", "UUID")
      VALUES (${orderReceiptId}, ${type}, ${dto.fileName}, ${buffer}, ${thumbnail}, 1, now(), ${userId}, 0, gen_random_uuid())
      RETURNING "RecId" as id, "FileName" as "fileName", "InsertedAt" as "uploadedAt"
    `);
    return sanitizeRawRow({ ...rows[0], fileType: ext.toUpperCase() });
  }

  async content(orderReceiptId: number, attachmentId: number): Promise<{ fileName: string; mimeType: string; buffer: Buffer }> {
    const rows = await this.prisma.$queryRaw<any[]>(Prisma.sql`
      SELECT "FileName" as "fileName", "Attachment" as "attachment"
      FROM "IM_OrderReceiptAttachment" WHERE "RecId" = ${attachmentId} AND "OrderReceiptId" = ${orderReceiptId} AND "IsDeleted" = 0
    `);
  ...
  async remove(orderReceiptId: number, attachmentId: number, userId: number) {
    const result = await this.prisma.$executeRaw`
      UPDATE "IM_OrderReceiptAttachment" SET "IsDeleted" = 1, "DeletedAt" = now(), "DeletedBy" = ${userId}
      WHERE "RecId" = ${attachmentId} AND "OrderReceiptId" = ${orderReceiptId}
    `;
