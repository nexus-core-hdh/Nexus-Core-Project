...
export class YarnCardController {
  constructor(
    private readonly svc: YarnCardService,
  ...
  // Preview-only — lets the Create screen display the code it's about to get before Save
  // is ever pressed. Best-effort: the code actually assigned at Save time is decided fresh
  // (and retried on collision) in create(), so a concurrent save elsewhere can still shift
  // it — this only avoids a blank/misleading Code field while the user is still typing.
  ...
  // its existing Type column (see yarn-card-attachments.service.ts). Declared before the
  // generic :id/:tab routes below since they'd otherwise match this same path shape first.
