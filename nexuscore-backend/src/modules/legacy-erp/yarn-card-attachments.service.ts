...
// IM_ItemAttachment already has a Type column (bare smallint, no FK/CHECK, no seed data —
// same "empty table, no documented convention" situation account-attachments.service.ts
// found for FI_AccountAttachment). Reusing the exact same Type=1 (document) / Type=2
// (picture) convention already established there, so "Attachments" and "Picture Gallery"
// share one real table instead of inventing a second one for the gallery.
...
@Injectable()
export class YarnCardAttachmentsService {
  constructor(private readonly prisma: PrismaService) {}

  async list(itemId: number, kind: 'document' | 'picture') {
    ...
    const rows = kind === 'picture'
      ? await this.prisma.$queryRaw<any[]>(Prisma.sql`
          ...
          FROM "IM_ItemAttachment"
          WHERE "InventoryId" = ${itemId} AND "Type" = ${type} AND "IsDeleted" = 0
  ...
  async upload(itemId: number, dto: UploadDto, userId: number) {
    ...
    const rows = await this.prisma.$queryRaw<any[]>(Prisma.sql`
      INSERT INTO "IM_ItemAttachment" ("InventoryId", "Type", "FileName", "Attachment", "Thumbnail", "InUse", "InsertedAt", "InsertedBy", "IsDeleted", "UUID")
      VALUES (${itemId}, ${type}, ${dto.fileName}, ${buffer}, ${thumbnail}, 1, now(), ${userId}, 0, gen_random_uuid())
      RETURNING "RecId" as id, "FileName" as "fileName", "InsertedAt" as "uploadedAt"
    `);
    return sanitizeRawRow({ ...rows[0], fileType: ext.toUpperCase() });
  }

  async content(itemId: number, attachmentId: number): Promise<{ fileName: string; mimeType: string; buffer: Buffer }> {
    const rows = await this.prisma.$queryRaw<any[]>(Prisma.sql`
      SELECT "FileName" as "fileName", "Attachment" as "attachment"
      FROM "IM_ItemAttachment" WHERE "RecId" = ${attachmentId} AND "InventoryId" = ${itemId} AND "IsDeleted" = 0
    `);
  ...
  async remove(itemId: number, attachmentId: number, userId: number) {
    const result = await this.prisma.$executeRaw`
      UPDATE "IM_ItemAttachment" SET "IsDeleted" = 1, "DeletedAt" = now(), "DeletedBy" = ${userId}
      WHERE "RecId" = ${attachmentId} AND "InventoryId" = ${itemId}
    `;
